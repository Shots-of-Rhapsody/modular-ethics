...
const Engine = (() => {
  ...
  // VAX: expected life-years saved = (deaths averted) × (years_left)
  function vaxExpectedLifeYearsSaved(scn) {
    ...
    function lySaved(actionId) {
      ...
      return v(deathsAverted(H, vH) * H.years_left + deathsAverted(E, vE) * E.years_left);
    }
  ...
  function evacRawls(scn) {
    ...
    const perPerson = {
      ...
      b2_west_plus_one_east: () => ({
        D1:(1/3)*pNow+(2/3)*pE, D2:(1/3)*pNow+(2/3)*pE, D3:(1/3)*pNow+(2/3)*pE,
        D4:pNow, D5:pNow
      }),
      b3_mixed_break_promise: () => ({
        D1:pNow,
        D2:0.5*pNow+0.5*pE, D3:0.5*pNow+0.5*pE,
        D4:0.5*pNow+0.5*pW, D5:0.5*pNow+0.5*pW
      }),
    };
